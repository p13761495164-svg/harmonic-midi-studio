...
export default function Home() {
  ...
  useEffect(() => {
    const blockBrowserWheelZoom = (event: WheelEvent) => {
      if (event.ctrlKey || event.metaKey) event.preventDefault();
    };
    const blockBrowserGestureZoom = (event: Event) => {
      event.preventDefault();
    };
    window.addEventListener("wheel", blockBrowserWheelZoom, { passive: false, capture: true });
    document.addEventListener("gesturestart", blockBrowserGestureZoom, { passive: false, capture: true });
    document.addEventListener("gesturechange", blockBrowserGestureZoom, { passive: false, capture: true });
    document.addEventListener("gestureend", blockBrowserGestureZoom, { passive: false, capture: true });
    return () => {
      window.removeEventListener("wheel", blockBrowserWheelZoom, true);
      document.removeEventListener("gesturestart", blockBrowserGestureZoom, true);
      document.removeEventListener("gesturechange", blockBrowserGestureZoom, true);
      document.removeEventListener("gestureend", blockBrowserGestureZoom, true);
    };
  }, []);
